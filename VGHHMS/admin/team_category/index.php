...
<?
	...
	$para = array();
	$order = array("sort"=>"a", "id"=>"d");
	$sql = "SELECT id, detail, title, inuse FROM `team_category` WHERE 1 = 1 ";
	if($qry_keyword!=''){
		$para["keyword"] = '%'.$qry_keyword.'%';
		$sql .= "AND (title LIKE :keyword OR en_title LIKE :keyword OR detail LIKE :keyword OR en_detail LIKE :keyword) ";
	}
