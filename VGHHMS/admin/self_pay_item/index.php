...
<?
	...
	$para = array();
	$order = array("c.sort"=>"a", "c.id"=>"a", "i.sort"=>"a", "i.id"=>"a");
	$sql = "SELECT c.name AS class_name, i.id, i.name, i.price, i.inuse 
			FROM self_pay_item i 
			INNER JOIN self_pay_item_class c ON i.class_id = c.id 
			WHERE 1 = 1 ";
	if($qry_class!=''&&$qry_class!='0'){
		$para["class_id"] = $qry_class;
		$sql .= "AND i.class_id = :class_id ";
	}
	if($qry_keyword!=''){
		$para["keyword"] = '%'.$qry_keyword.'%';
		$sql .= "AND(c.name LIKE :keyword OR i.name LIKE :keyword) ";
	}
						...
						<div class="col-md-6">					
							<select id="sel_class" name="sel_class" class="form-control" size="1">
								...
								<? foreach($data_class as $row){ ?>
									<option value="<? echo $row["id"]; ?>" <? echo $qry_class==$row["id"]?"selected":""; ?>><? echo $row["name"]; ?></option>
								<? } ?>
							</select>
						</div>
					</div>
