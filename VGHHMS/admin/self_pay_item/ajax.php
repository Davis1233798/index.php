...
switch($fn){
	...
	case "index_sort":		
		...
		$sql = 'SELECT class_id, sort FROM self_pay_item WHERE id = :id';
		$data = $db->doselect_first($sql, $para);

		if($sort_diff>0){
			// 若排序數字變大，檢查是否超過此分類的最大值
			$para = array('class_id'=>$data['class_id']);
			$sql = 'SELECT MAX(sort) AS sort FROM self_pay_item WHERE class_id = :class_id';
		...
		$db->dosort('self_pay_item', 'id', 'sort', $id, $sort_diff, 'class_id', $data['class_id']);
