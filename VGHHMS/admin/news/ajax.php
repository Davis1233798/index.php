...
switch($fn){
	...
	case "del":
		...
		$file1 = $_env["site_upload_path"]."news/".$data["filename"];
		$file2 = $_env["site_upload_path"]."news/".$data["img"];
		if(file_exists($file1)&&is_file($file1)) unlink($file1);
		if(file_exists($file2)&&is_file($file2)) unlink($file2);
		// remove_dir($_env["site_upload_path"]."news/".$id."/");
		
		$db->dodelete("news", "id", array($id));
