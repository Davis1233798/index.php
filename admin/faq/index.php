...
<?
	...
	$para = array();
	$order = array("f.sort"=>"a", "f.id"=>"d");
	$sql = "SELECT f.id, f.category_id, f.question, f.inuse, 
			c.title AS cate_title 
			FROM faq f 
			INNER JOIN faq_category c ON f.category_id = c.id 
			WHERE 1 = 1 ";
	/*$sql = "SELECT f.id, f.category_id, f.question, f.inuse
			FROM faq f 
			WHERE 1 = 1 ";		*/
	...
	if($qry_keyword!=''){
		...
		$sql .= "AND (f.question LIKE :keyword OR f.answer LIKE :keyword) ";
	}
	$query = $db->doselect_page($sql, $para, $order, $page);

	$sql = "SELECT id, title FROM faq_category ORDER BY sort, id";
			...
			<form id="index_qry_form" method="POST">
				<div class="col-md-8">
					<div class="form-group row">
						<label class="col-md-2 form-control-label" for="text-input">問題類型</label>
						<div class="col-md-4">					
