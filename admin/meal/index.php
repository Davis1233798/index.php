...
<main class="main">
    <div class="container-fluid">
        ...
        $sql = "SELECT c.meal_id, c.meal_name , c.price, c.vegetarian, c.remark , c.enable 
			    FROM meal c";
                                    ...
                                    <table class="table table-bordered table-striped table-condensed">
                                        ...
                                        <? foreach($query["data"] as $row){ ?>
                                            <tr id="tr_<? echo $row["id"] ?>" data-id="<? echo $row["id"] ?>">
                                                <td><? echo $row['meal_id'] ?></td>
                                                <td><? echo $row["meal_name"] ?></td>
                                                <td>
                                                    <? if($row['vegetarian']=='1'){ ?><span class="label label-success">葷食</span><? } ?>
                                                    <? if($row['vegetarian']=='2'){ ?><span class="label label-warning">素食</span><? } ?>
                                                </td>
                                                <td><? echo $row["remark"] ?></td>
                                                <td><? echo $row["price"] ?></td>
                                                <td>
                                                    <? if($row['enable']=='1'){ ?><span class="label label-success">啟用</span><? } ?>
                                                    <? if($row['enable']=='0'){ ?><span class="label label-default">停用</span><? } ?>
                                                </td>
                                                <td>
                                                    <button type="button" class="btn btn-primary" onclick="javascript:location.href='edit.php?id=<? echo $row['meal_id'] ?>'">編輯</button>
