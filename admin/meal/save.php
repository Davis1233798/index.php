...
try{
    ...
    switch($fn){
        ...
        case "add":
            $para = array(
                //Jack 20220217 Add for insert meal
                "ID"=>get_post("meal_ID"),
                "meal_ID"=>get_post("meal_ID"),
                "meal_name"=>get_post("meal_name"),
                "price"=>get_post("price"),
                "vegetarian"=>get_post("vegetarian",0),
                "remark"=>get_post("remark"),
                "inuse"=>get_post("enable", 0)
            );

            if($para["meal_name"]=='')throw new exception("請輸入代餐名稱");
            if($para["meal_ID"]=='')throw new exception("請輸入代餐編號");
            if($para["price"]=='')throw new exception("請輸入代餐價格");



            $lastid = $db->doinsert("meal", $para);//寫入資料表，取得id
