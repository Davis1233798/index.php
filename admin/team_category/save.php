...
switch($fn){
	...
	case "add":
		$para = array(
			...
			"detail"=>get_post("txt_detail"),
			"en_detail"=>get_post("txt_en_detail"),
			"inuse"=>get_post("rd_inuse", 0),
		);
		
		if($para["title"]=='') throw new exception("請輸入中文黑字大標題");
		
		$lastid = $db->doinsert("team_category", $para);//寫入資料，取得id
	...
	case "edit":
		...
		$db->doupdate("team_category", "where id = :id", $para, array("id"));
